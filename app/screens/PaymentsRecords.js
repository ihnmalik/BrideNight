...
export class PaymentsRecords extends Component {
  constructor(props) {
    super(props);
    this.state = {
      users: InboxOrders,
    ...
    if(this.state.lang == 'ar') {
      return (
        <View key={item.id} style={{ padding: 3, width: '100%', borderBottomWidth: 1, borderBottomColor: '#eee'}}>
          <TouchableOpacity activeOpacity={.8} style={{flex: 1, flexDirection: 'row', justifyContent: 'flex-end'}} onPress={() => this.props.navigation.navigate('PaymentRecordDetail')}>  
            <View style={{flex: 1, alignItems: 'flex-end', padding: 8}}>
              <Text style={{fontSize: 17}}>بيان الدفعة</Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(PaymentsRecords)